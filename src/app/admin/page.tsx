...
export default function AdminPage() {
  ...
  async function uploadMusic(file: File) {
    ...
    try {
      ...
      const res = await fetch('/api/admin/musicas', { method: 'POST', body: formData })
      if (!res.ok) throw new Error('Upload failed')
      const data = await res.json()
      const updated = [...musicUrls, data.url]
          ...
          <div className="space-y-4">
            ...
            <div className="bg-beige/20 rounded-xl p-3 md:p-4 space-y-3">
              <label className="block font-sans text-xs text-dark/50 uppercase tracking-wider flex items-center gap-2">
                <Music className="w-4 h-4 text-gold" /> Músicas de fundo (MP3)
              </label>
              ...
              <div className="space-y-2">
                ...
                {musicUrls.map((url, i) => (
                  <div key={i} className="flex items-center gap-2 bg-white rounded-lg border border-beige p-2">
                    ...
                    <p className="flex-1 text-xs text-dark/70 font-sans truncate">{decodeURIComponent(url.split('/').pop()?.split('?')[0] || url)}</p>
                    <button
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (file) { uploadMusic(file); e.target.value = '' }
                  }}
                  className="hidden"
                />
                <button
                ...
                >
                  ...
                  {uploadingMusic ? 'Enviando...' : 'Adicionar MP3'}
                </button>
              </div>
            </div>
